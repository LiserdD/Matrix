...
function MultiMatrix() {
	...
	function Multi(mas1, mas2, mas3) {
		var resultF = document.querySelector('#result');

		if (collumsMat1 == rowsMat2){
			for (var i = 0; i < rowsMat1; i++) {   

				for (var j = 0; j < collumsMat2; j++) {
					mas3[i][j] = 0;

					for (var k = 0; k < collumsMat1; k++) {	
						mas3[i][j] += mas1[i][k] * mas2[k][j];	 
					}			
				}
			}

			for(var i = 0; i < collumsMat2; i ++) {
				resultF.innerHTML +='<div class="flexCnt"></div>'; 
				for(var j = 0; j < rowsMat1; j ++) {
					$('.flexCnt').last().html(function(indx, oldHtml) {
							return oldHtml + '<div class="result-item">'+ Math.round(mas3[j][i] * 100) / 100  +'</div><br>' ;
						});	
				}
			...
			resultF.innerHTML += '<div class="p1">Столбцы первой матрицы должны быть равны рядам второй</div>';
		} 
	}//УМНОЖЕНИЕ Принимает 3 матрыци, [row][collum] берем из замыкания, умножаем и записываем в '#result'
	...
	function GreatMatrField() {
		...
		rowsMat2 = $('#matr2-rows').val();
		collumsMat2 = $('#matr2-collums').val();

		MtrAdd(rowsMat1, collumsMat1, '#matr1');
		MtrAdd(rowsMat2, collumsMat2, '#matr2');
	...
	var rowsMat1, rowsMat2, collumsMat1, collumsMat2;
	...
	$("#rezBttn").on('click', function(){
		...
		$('#matr2').children('.matrInp').each(function(indx, element){
		 	inpVal_2.push($(element).val());
		});
		
		MatrixGreat(mas_1, rowsMat1, collumsMat1, inpVal_1);
		MatrixGreat(mas_2, rowsMat2, collumsMat2, inpVal_2);
		MatrixGreat(mas_3, rowsMat1, collumsMat2, inpVal_2);

		Multi(mas_1, mas_2, mas_3);
		Maxwidth();
