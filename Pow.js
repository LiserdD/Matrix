...
function MultiMatrix() {
	...
	function Multi(mas1, mas2, mas3, size) {
	...
	function Pow(mas1, mas2, mas3, ans, n) {
		var resultF = document.querySelector('#result');
		for (var i = 0; i < length; i++) {
			for (var j = 0; j < length; j++) {
				mas3[i][j] = mas2[i][j] = mas1[i][j];
				ans[i][j] = 0;
			}
		}
		for (var it = 1; it < n; it++) {
			Multi(mas1, mas2, mas3, length);
	...
	$("#rezBttn").on('click', function() {
		...
		MatrixGreat(mas_1, length, length, inpVal_1);
		MatrixGreat(mas_2, length, length, inpVal_1);
		MatrixGreat(mas_3, length, length, inpVal_1);
		MatrixGreat(ans, length, length, inpVal_1);

		var n =$('#mltnumber').val();

		Pow(mas_1, mas_2, mas_3, ans, n);
		Maxwidth();
