...
function sumOrMin() {
	...
	function act(mas1, mas2, mas3, act) {
	...
	function GreatMatrField() {
		...
		MtrAdd(rowsMat1, collumsMat1, '#matr2');

		$('#result').html(function(indx, oldHtml) {
			return '';
		}); 
		inpFocus();
	}// при клике на кнопку создаем матрицы [][], с значениями row, collum взятых с <select>. Вызывает inpFocus() MtrAdd()
	...
	GreatMatrField();
	$(".matrixSize").change(GreatMatrField);
	...
	$("#rezBttn").on('click', function() {
		...
		MatrixGreat(mas_2, rowsMat1, collumsMat1, inpVal_2);
		MatrixGreat(mas_3, rowsMat1, collumsMat1, inpVal_2);

		var selectAct = $('#selectAct').val();
		act(mas_1, mas_2, mas_3, selectAct);
		Maxwidth();
